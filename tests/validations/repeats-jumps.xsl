...
<stylesheet xmlns="http://www.w3.org/1999/XSL/Transform"
   ...
   <mode name="group.d6e10"
         on-no-match="shallow-skip"
         streamable="false"
         use-accumulators=""/>
   <template match="* | root()" mode="group.d6e10" priority="-10">
      <apply-templates select="@*" mode="#current"/>
      <apply-templates select="node()" mode="#current"/>
   </template>
   <template match="sound[@tocoda]" mode="group.d6e10" priority="2">
      ...
      <variable name="Q{http://dmaus.name/ns/2023/schxslt}rule-context"
                ...
                select="."/>
      <choose>
         <when test="'d6e10' = $Q{http://dmaus.name/ns/2023/schxslt}pattern">
            <next-match/>
         </when>
         <otherwise>
            ...
            <for-each select=".">
               <variable name="tocoda" select="@tocoda"/>
               <choose>
                  <when test="Q{http://www.w3.org/2005/xpath-functions/map}get($Q{http://dmaus.name/ns/2023/schxslt}severity, 'd6e16') ge Q{http://dmaus.name/ns/2023/schxslt}numeric-severity('info')">
                     <if test="not(count(//sound[@coda = $tocoda]) = 1)">
                        <variable name="failed-assert" as="element(svrl:failed-assert)">
                           <svrl:failed-assert test="count(//sound[@coda = $tocoda]) = 1"
                                               patternId="tocoda-coda-matching">
                              <where-populated>
                                 <attribute name="location"
                                            select="path($Q{http://dmaus.name/ns/2023/schxslt}rule-context)"/>
                              </where-populated>
                              <svrl:text>
        sound element with @tocoda="<for-each select=".[1]">
                                    <value-of select="$tocoda"/>
                                 </for-each>"
        must have exactly one corresponding sound element with @coda="<for-each select=".[1]">
                                    <value-of select="$tocoda"/>
                                 </for-each>".
      </svrl:text>
                           </svrl:failed-assert>
            ...
            <next-match>
               <with-param name="Q{http://dmaus.name/ns/2023/schxslt}pattern"
                           as="Q{http://www.w3.org/2001/XMLSchema}string*"
                           select="('d6e10', $Q{http://dmaus.name/ns/2023/schxslt}pattern)"/>
            </next-match>
         </otherwise>
      </choose>
   </template>
   <template match="sound[@coda]" mode="group.d6e10" priority="1">
      <param name="Q{http://dmaus.name/ns/2023/schxslt}pattern"
             as="Q{http://www.w3.org/2001/XMLSchema}string*"
             select="()"/>
      <variable name="Q{http://dmaus.name/ns/2023/schxslt}rule-context"
                ...
                select="."/>
      <choose>
         <when test="'d6e28' = $Q{http://dmaus.name/ns/2023/schxslt}pattern">
            <next-match/>
         </when>
         <otherwise>
            <variable name="Q{http://dmaus.name/ns/2023/schxslt}severity"
            ...
            <for-each select=".">
               <variable name="coda" select="@coda"/>
               <choose>
                  <when test="Q{http://www.w3.org/2005/xpath-functions/map}get($Q{http://dmaus.name/ns/2023/schxslt}severity, 'd6e34') ge Q{http://dmaus.name/ns/2023/schxslt}numeric-severity('info')">
                     <if test="not(count(//sound[@tocoda = $coda]) = 1)">
                        <variable name="failed-assert" as="element(svrl:failed-assert)">
                           <svrl:failed-assert test="count(//sound[@tocoda = $coda]) = 1"
                                               patternId="coda-tocoda-matching">
                              <where-populated>
                                 <attribute name="location"
                                            select="path($Q{http://dmaus.name/ns/2023/schxslt}rule-context)"/>
                              </where-populated>
                              <svrl:text>
        sound element with @coda="<for-each select=".[1]">
                                    <value-of select="$coda"/>
                                 </for-each>"
        must have exactly one corresponding sound element with @tocoda="<for-each select=".[1]">
                                    <value-of select="$coda"/>
                                 </for-each>".
      </svrl:text>
                           </svrl:failed-assert>
            ...
            <next-match>
               <with-param name="Q{http://dmaus.name/ns/2023/schxslt}pattern"
                           as="Q{http://www.w3.org/2001/XMLSchema}string*"
                           select="('d6e28', $Q{http://dmaus.name/ns/2023/schxslt}pattern)"/>
            </next-match>
         </otherwise>
      </choose>
   </template>
   <template match="barline" mode="group.d6e10" priority="0">
      <param name="Q{http://dmaus.name/ns/2023/schxslt}pattern"
             as="Q{http://www.w3.org/2001/XMLSchema}string*"
             select="()"/>
      <variable name="Q{http://dmaus.name/ns/2023/schxslt}rule-context"
                ...
                select="."/>
      <choose>
         <when test="'d6e45' = $Q{http://dmaus.name/ns/2023/schxslt}pattern">
            <next-match/>
         </when>
         <otherwise>
            <variable name="Q{http://dmaus.name/ns/2023/schxslt}severity"
            ...
            <for-each select=".">
               <choose>
                  <when test="Q{http://www.w3.org/2005/xpath-functions/map}get($Q{http://dmaus.name/ns/2023/schxslt}severity, 'd6e49') ge Q{http://dmaus.name/ns/2023/schxslt}numeric-severity('info')">
                     <if test="not(not(coda and segno))">
                        <variable name="failed-assert" as="element(svrl:failed-assert)">
                           <svrl:failed-assert test="not(coda and segno)" patternId="barline-segno-coda-cooccur">
                              <where-populated>
                                 <attribute name="location"
                                            select="path($Q{http://dmaus.name/ns/2023/schxslt}rule-context)"/>
                              </where-populated>
        ...
        coda and segno elements cannot co-occur within a barline.
            ...
            <next-match>
               <with-param name="Q{http://dmaus.name/ns/2023/schxslt}pattern"
                           as="Q{http://www.w3.org/2001/XMLSchema}string*"
                           select="('d6e45', $Q{http://dmaus.name/ns/2023/schxslt}pattern)"/>
            </next-match>
         </otherwise>
      </choose>
   </template>
   <template match="root()" as="element(svrl:schematron-output)">
      <svrl:schematron-output phase="#ALL" title="Repeats and jumps"><!--SchXslt2 1.11.1-->
         <try>
   ...
   <template match="root()"
      ...
      <apply-templates select="root()" mode="group.d6e10"/>
   </template>
